...
class AppStateProvider extends ChangeNotifier {
    ...
    _favoritesSubscription = _firestoreService
        ...
        .listen(
          (favorites) {
            ...
            print(
              'Favorites updated: ${_favorites.map((e) => e.title).toList()}',
            );
            notifyListeners();
          },
          onError: (e) {
  ...
  void toggleFavorite(Anime anime) {
    if (isFavorite(anime.malId)) {
      print('Removing from favorites: ${anime.title}');
      removeFavorite(anime.malId);
    } else {
      print('Adding to favorites: ${anime.title}');
      addFavorite(anime);
    }
  }
  ...
  Future<void> addFavorite(Anime anime) async {
    ...
    if (user != null) {
      print('Adding favorite for user ${user.uid}: ${anime.title}');
      try {
  ...
  Future<void> removeFavorite(int malId) async {
    final user = FirebaseAuth.instance.currentUser;
    if (user != null) {
      print('Removing favorite for user ${user.uid}, animeId: $malId');
      try {
